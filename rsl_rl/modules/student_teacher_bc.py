...
class BCStudentTeacher(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        num_student_obs = 0
        assert "policy" in self.obs_groups and "teacher" in self.obs_groups, \
            "obs_groups must contain 'policy' and 'teacher' keys"
        num_student_obs = obs[obs_groups["policy"][0]].shape[-1]
        num_teacher_obs = obs[obs_groups["teacher"][0]].shape[-1]
    ...
    def sample_teacher(self, obs: TensorDict) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self.noise_std_type == "scalar":
                ...
                raise ValueError(f"Unknown standard deviation type: {self.noise_std_type}. Should be 'scalar' or 'log'")
            # Create distribution
            teacher_distribution = Normal(mean, std)
    ...
    def get_previliege_obs(self,obs: TensorDict) -> torch.Tensor:
        obs_list = [obs[obs_group] for obs_group in self.obs_groups["previliege"]]
